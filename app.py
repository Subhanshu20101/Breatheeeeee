...
def predict():
    if 'username' in session:
        ...
        lis = ['AKGC417L','LittC2SE','Litt3200','Meditron']

        if (not audio) :
            return render_template('index.html', error_text="No Audio File Found ", user_logged_in = True)
                
        if audio.filename.split('_')[-1].split('.')[0] not in lis:
            return render_template('prediction.html', prediction_text="No Breathing Pattern Found", prediction_confidence="0.0")
        ...
        return render_template('prediction.html', prediction_text="{}".format(output[0]),
                               prediction_confidence="{}".format(round(output[1] * 100, 2)))
    else:
